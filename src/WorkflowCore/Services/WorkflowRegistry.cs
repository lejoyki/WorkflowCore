...
namespace WorkflowCore.Services
{
    public class WorkflowRegistry : IWorkflowRegistry
    {
        ...
        public void DeregisterWorkflow(string workflowId, int version)
        {
            ...
            lock (_registry)
            {
                ...
                if (_lastestVersion[workflowId].Version == version)
                {
                    ...
                    var latest = _registry.Values.Where(x => x.Name == workflowId).OrderByDescending(x => x.Version).FirstOrDefault();
                    if (latest != default)
        ...
        public void RegisterWorkflow(IWorkflow workflow)
        {
            ...
            var def = builder.Build(workflow.Name, workflow.Version);
            RegisterWorkflow(def);
        }

        public void RegisterWorkflow(WorkflowDefinition definition)
        {
            if (_registry.ContainsKey($"{definition.Name}-{definition.Version}"))
            {
                throw new InvalidOperationException($"Workflow {definition.Name} version {definition.Version} is already registered");
            }

            lock (_registry)
            {
                _registry[$"{definition.Name}-{definition.Version}"] = definition;
                if (!_lastestVersion.ContainsKey(definition.Name))
                {
                    _lastestVersion[definition.Name] = definition;
                    return;
                }

                if (_lastestVersion[definition.Name].Version <= definition.Version)
                    _lastestVersion[definition.Name] = definition;
            }
        }
